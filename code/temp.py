...
class RRT:
    ...
    def new_state(self, q_near, q_rand, env):
        ...
        for i in range(1, len(trajectories)):
            d = self.distance(trajectories[i][-1], q_rand)
            if d < dmin:
                nnear = i
        ...
        if collision == False:
            (x,y,theta) = trajectories[nnear]
            q_new = node(x,y,theta)
            self.nodes.append(q_new)                        #Add node to our tree
            q_near.children.append(q_new, control[nnear])   #Append child with control necessary to reach that node


    #Return list of x,y,theta points generated taking a certain curve
    def trajectory(self, n, control):
        ...
        for i in range(1,int(unit_time/dt)):
			dtheta = (theta[i-1]+speed*math.tan(turn)/1.9*dt)
			dx = (x[i-1]+speed*math.cos(theta[i-1])*dt)
            dy = (y[i-1]+speed*math.sin(theta[i-1])*dt)
            di = node(dx,dy, dtheta)
            path.append(di)
        return path
